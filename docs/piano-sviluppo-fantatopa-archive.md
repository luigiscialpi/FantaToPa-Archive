...
- **Variante "flat"** (2014-15, 2016-17, 2017-18): pagine HTML server-renderizzate con
  ...
  formattazione diversa, tollerata con `\s*` nei regex invece di due parser separati.
  Adapter `FlatHtml{Standings,Calendar,Roster,Lineup}Adapter`
  (`packages/ingestion/adapters/html-legacy/{standings,calendar,roster,lineup}.ts`),
  condivisi dalle tre stagioni. La rosa arriva in due forme diverse a seconda della
  stagione: un file per squadra (`dettaglio-rosa/<squadra>/<id>.htm`, 2016-17/2017-18)
  o un unico file aggregato con una tabella per squadra (`tutte-le-rose.html`,
  2014-15) — stesso adapter, due funzioni di parsing (`parseFlatTeamTableRows`
  condivisa, `parseFlatAggregateRoster` solo per il caso aggregato). La 2014-15 è anche
  l'unica delle tre senza Coppa importabile: il menu competizione esiste nel mirror ma
...
1. **Storico** (card "Ultima stagione" in Home, rinominata "Storico" nel Profilo Squadra dedicato — vedi Fase 4) — grafico a linee del piazzamento per stagione partecipata, calcolato da `getStandingHistory` (solo Campionato: una fase finale Coppa a eliminazione diretta non ha una riga standings da cui derivare un piazzamento). Con una sola stagione nota (tipico delle stagioni manuali 2004-05→2012-13, dove solo il podio è documentato) una linea non è disegnabile: mostra il singolo dato come testo ("Stagione X: N°") invece di restare vuota. Dal 2° dato in su diventa il grafico vero e proprio; il tap su un punto mostra un tooltip con stagione/posizione invece di navigare (Client Component).
2. **Bacheca** — conteggio titoli vinti nel tempo, derivato da `standings` (posizione 1/2/3 per Campionato, vincitore dedotto dall'ultima giornata per la fase finale di Coppa): "Campionati", "Coppe", "2° posto"/"3° posto" separati (questi ultimi due solo Campionato — la fase finale di Coppa non produce mai una riga standings, nessun "2°/3°" deducibile come il vincitore) invece di un unico numero che nasconde quale competizione/piazzamento.
3. **Stagioni disputate** — conteggio da `team_seasons` (una riga per stagione, indipendente da Campionato/Coppa). Per le stagioni manuali 2004-05→2012-13 (nessuna riga `team_seasons`, solo un podio in `standings` — vedi sopra), se la squadra vi compare il conteggio le include comunque e l'etichetta diventa "Almeno N" invece del numero secco: resta un minimo, non un totale certo, perché una stagione manuale senza podio per quella squadra non lascia traccia recuperabile in nessuna fonte già importata.
...
**Stagioni con solo un podio manuale (nota storica, senza calendario/formazioni reali — es. 2004-05→2012-13, precedenti al primo mirror disponibile)**: `getSeasons` espone `hasSchedule: boolean` (vero se la stagione ha almeno una giornata reale in `matchdays`). Il selettore in header le esclude del tutto; la galleria stagioni in home continua a mostrare la card (podio/vincitore Coppa restano un dato interessante), ma senza `<Link>` — click disabilitato invece di portare a classifica/calendario/formazioni vuoti. Calcolato dinamicamente da dati reali, non un flag statico: se in futuro una di queste stagioni viene arricchita con un calendario vero (import o pagina admin), torna automaticamente navigabile senza altre modifiche.
...
**Cose che restano invariate dal v1, confermate**: rosa/profilo giocatore trasversale, ricerca full-text nativa Postgres (niente Algolia, sarebbe over-engineering per questi volumi), generazione statica per stagioni chiuse — vedi sezione 3, punto 5. Il profilo storico squadra e l'albo d'oro sono oggi completati — vedi Fase 4.
...
**Home personalizzata** (pannello squadra + vetrina generale + galleria stagioni, sezione 10) *(completata)*. **Albo d'oro** e **Statistiche** *(completate)*: entrambe route top-level (`/albo-doro`, `/statistiche`, non sotto `stagioni/[season]/**`), raggiungibili da una nav globale persistente (`GlobalNav`, dentro `AppHeader`: Home/Albo d'Oro/Statistiche) invece che come tab per-stagione — scelta esplicita dell'utente in revisione del mockup originale. Albo d'oro riusa `SeasonPodium`/`CupWinnerLine` già scritti per la galleria Home, stesso trattamento click-through/disattivo (`hasSchedule`) per le stagioni senza calendario reale. Statistiche calcola il confronto punti/fantapunti tra due squadre da `matches` (nessuna tabella nuova); i filtri (stagione/competizione/squadre/tipo dato) usano un `<form>` non controllato con pulsante "Aggiorna" esplicito invece di `onChange` → `router.push()` immediato, per non far partire una query ad ogni singola selezione; il grafico SVG ha assi con etichette e tooltip al tap (client component, a differenza dello sparkline Home che è Server Component). **Profilo Squadra** *(completata)*: route top-level (`/profilo-squadra`) con selettore libero di qualunque squadra (non solo quella dell'utente, a differenza del pannello Home) — riusa `TeamPanelSection` così com'è, stesso contenuto del pannello Home (card Storico/Bacheca/Stagioni disputate/avversari/record/fedelissimi), nessuna query nuova per il pannello in sé. Restano da fare: profilo giocatore multi-stagione, ricerca.
...
**Fase 7 — Bonus/malus storici da fonte web (2020-21 → 2024-25)** *(pilota 2024-25 costruito, in attesa di esecuzione fetch+import — analisi in `docs/bonus-storici-fantacalcio-it.md`, dettagli operativi in memoria repo `ingestion-pilot-import.md`)*
Le 5 stagioni classico/mantra intermedie hanno solo voto/fantavoto da xlsx, senza bonus/malus granulari (a differenza di 2025-26 e 2017-18, sezione 7.1). Fonte confermata: pagina pubblica `https://www.fantacalcio.it/voti-fantacalcio-serie-a/{stagione}/{giornata}`, non login-gated, un adapter dedicato (`FantacalcioItBonusAdapter`) + due script (`fetch-bonus-fantacalcio-it.ts`, `import-bonus-fantacalcio-it.ts`) riusano senza modifiche l'architettura esistente (`bonus_kinds`/`player_matchday_bonuses`/`matchday_bonus_sources`). Colonne dirette: gol fatti/subiti, autogol, rigori segnati/sbagliati/parati, assist, player of the match, subentrato/uscito. Ammonizione/espulsione non sono una colonna dedicata ma si rilevano comunque in modo affidabile dalla classe CSS (`yellow-card`/`red-card`) del voto "Redazione Fantacalcio" — niente derivazione aritmetica dal delta voto/fantavoto, verificata e scartata perché il voto stesso è già abbassato dal cartellino. `portiere_imbattuto` invece **non è disponibile da questa fonte** (verificato empiricamente: portieri con clean sheet hanno voto === fantavoto), quindi non viene importato per queste 5 stagioni. Resta da eseguire il fetch+import reale per 2024-25 e, se l'esito è positivo, ripetere per 2020-21→2023-24; include anche la mappatura Coppa (`matchday_bonus_sources`) per 2022-23/2023-24/2024-25, non ancora popolata.
