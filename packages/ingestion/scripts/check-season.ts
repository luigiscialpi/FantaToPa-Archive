...
async function checkSeason(config: SeasonConfig): Promise<{ ok: boolean; issues: string[] }> {
  const issues: string[] = [];
  ...
  if (uncovered.length > 0) {
    issues.push(`squadre non coperte da TEAM_REGISTRY: ${uncovered.join(', ')}`);
  }
  console.log(`  squadre totali osservate: ${teamNames.size}, non coperte da TEAM_REGISTRY: ${uncovered.length}`);
...
async function main(): Promise<void> {
  ...
  for (const config of configs) {
    const { ok, issues } = await checkSeason(config);
