...
function parseTeamEntries(tableHtml: string, teamName: string, filePath: string): RosterImport['entries'] {
  ...
  while ((rowMatch = rowPattern.exec(bodyMatch[1]!))) {
    ...
    // 2011-12: il ruolo è dentro <p class="ybkg acenter3">P</p>
    // (variante del 2013-14 che usa <span class="role role-p">P</span>).
    const roleMatch =
      /<(?:p|span)\b[^>]*class="[^"]*\b(?:ybkg|gbkg|bbkg|rbkg)\b[^"]*"[^>]*>\s*([PDCA])\s*<\/(?:p|span)>/i.exec(
        cells[0]![1]!,
      );
...
export class Html2011RosterAdapter implements SourceAdapter<RosterImport> {
  ...
  async parse(input: unknown): Promise<RosterImport> {
    if (typeof input !== 'string') {
      throw new Error('Html2011RosterAdapter si aspetta un path file (string)');
    }
